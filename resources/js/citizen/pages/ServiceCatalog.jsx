            ...
            <div className="flex-1 w-full max-w-[1101px] mx-auto bg-white border-x border-gray-200 flex flex-col">
                ...
                <div className="flex flex-1 px-10">
                    <aside className="hidden md:block w-[220px] pr-6 py-8 border-r-2 border-gray-200">
                        <h3 className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-4">Categories</h3>
                    ...
                    <div className="flex-1 md:pl-8 py-8">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h2 className="text-[26px] font-bold text-gray-900">{selectedCategory ? selectedCategory.name : 'All Services'}</h2>
                            <span className="px-3 py-1 bg-gray-100 text-gray-500 rounded-full text-[13px] font-semibold">{services.length} services</span>
                        </div>

                        {loading ? (
                            <div className="py-10 text-center text-gray-500">Loading...</div>
                        ) : services.length === 0 ? (
                            <div className="py-10 text-center text-gray-500">No services found.</div>
                        ) : (
                                ...
                                {services.map((service, index) => (
                                    <Link key={service.id} to={`/services/${service.id}`} className={`flex flex-col md:flex-row md:items-center justify-between p-5 md:p-6 gap-4 md:gap-0 hover:bg-gray-50 transition group cursor-pointer ${index !== 0 ? 'border-t-[1.5px] border-gray-100' : ''}`}>
                                        <div className="flex-1 pr-6">
                                            ...
                                            <p className="text-[15px] text-gray-500">{service.description || 'No description available.'}</p>
                                        </div>
                                        <div className="flex items-center gap-6 md:gap-0">
                                            <div className="w-auto md:w-[110px] md:text-right flex flex-col justify-center">
                                                <span className="text-[13px] font-semibold text-gray-400 uppercase tracking-widest mb-1">Processing</span>
                                                <span className="text-[15px] font-semibold text-gray-700">{service.processing_time_days} days</span>
                                            </div>
                                            <div className="w-auto md:w-[80px] md:text-right flex flex-col justify-center md:ml-6 md:mr-6">
                                                <span className="text-[13px] font-semibold text-gray-400 uppercase tracking-widest mb-1">Fee</span>
                                                <span className="text-[15px] font-semibold text-gray-700">{service.fee > 0 ? `${new Intl.NumberFormat('en-US').format(service.fee)} ₫` : 'Free'}</span>
                                            </div>
                                            <div 
                                            ...
                                            >
                                                <span className="text-[15px] font-semibold">Apply</span>
